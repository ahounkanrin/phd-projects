...
train_df["soft_label"] = traindf["soft_label"]
val_df["impath"] = valdf["impath"].apply(fullpath)
val_df["soft_label"] = valdf["soft_label"]
test_df["impath"] = testdf["impath"].apply(fullpath)
test_df["soft_label"] = testdf["soft_label"]

classes = [str(i) for i in range(1, 361)]
...
if args.is_training:
    ...
    restored_model.compile(loss='categorical_crossentropy',
                        optimizer=keras.optimizers.SGD(lr=0.01, decay=1e-6, momentum=0.9),
                        metrics=['accuracy'])
