...
pred_probs = model.predict_generator(generator=test_generator, verbose=1)
pred_indices = np.argmax(pred_probs, axis=-1)
pred_classes = [int(decode_class_indices[i]) for i in pred_indices]
gt = np.array(testdf["label"].tolist())
pred = np.array(pred_classes) 
...
with open("Hard_classification.txt", "w") as f:
...
for theta in thresholds:
    ...
    with open("Hard_classification.txt", "a") as f:
