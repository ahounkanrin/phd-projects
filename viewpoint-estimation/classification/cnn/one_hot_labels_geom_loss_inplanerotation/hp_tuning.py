...
def test_step(images, labels):
    ...
    #test_loss = tf.reduce_sum(test_loss)/images.shape[0]  
...
if args.is_training:
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        test_acc = np.mean(np.array(np.array([error <= 30  for error in errors])))

        with val_summary_writer.as_default():
            tf.summary.scalar("test_accuracy_at_30", test_acc, step=epoch)
            tf.summary.image("test_images", test_images, step=epoch, max_outputs=1)

        ckpt_path = manager.save()
        template = "Epoch {}, Test Accuracy: {:.4f}, ckpt {}\n\n"
        print(template.format(epoch+1, test_acc, ckpt_path))
