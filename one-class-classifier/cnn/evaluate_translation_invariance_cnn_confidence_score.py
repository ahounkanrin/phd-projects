...
checkpoint2 = tf.train.Checkpoint(model=model2, optimizer=optimizer)
checkpoint_dir2 = "/scratch/hnkmah001/phd-projects/one-class-classifier/cnn/checkpoints/"
manager2 = tf.train.CheckpointManager(checkpoint2, directory=checkpoint_dir2, max_to_keep=10)
checkpoint2.restore(manager2.checkpoints[-1]) 
...
for view_id in tqdm(range(0, 360), desc="\n{}".format(scan)):
    ...
    for ty in range(-20, 21, 5): 
        for tx in range(-20, 21, 5):
            ...
            confidence = model2(x_test2, training=False)      
