...
for view_id in range(0, 180, 10):
    ...
    plt.title(r"Confidence score heatmap ($\theta $ = {} degrees)".format(view_id))
    plt.xlabel("tx (pixles)")
    plt.ylabel("ty (pixels)")
    sns.heatmap(confidence_arr, xticklabels=tx_list, yticklabels=ty_list)
    plt.xlabel("tx (pixles)")
    plt.ylabel("ty (pixels)")
    plt.savefig("hm_conf_score_{}.png".format(view_id))
