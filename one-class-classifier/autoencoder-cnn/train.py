...
def test_step(images, labels):
    ...
    test_loss = oneClassMSE(predictions)
...
train_accuracy = tf.keras.metrics.BinaryAccuracy(name='train_accuracy', dtype=None, threshold=0.90)
test_accuracy = tf.keras.metrics.BinaryAccuracy(name='val_accuracy', dtype=None, threshold=0.90)
...
checkpoint_dir = "/scratch/hnkmah001/phd-projects/one-class-classifier/autoencoder-cnn/checkpoints/"
...
train_logdir = "/scratch/hnkmah001/phd-projects/one-class-classifier/autoencoder-cnn/logs/train"
val_logdir = "/scratch/hnkmah001/phd-projects/one-class-classifier/autoencoder-cnn/logs/val"
...
for epoch in range(args.epochs):
    for images, _ in train_data:
        ...
        #print("INFO:", images.shape)
        #images = tf.image.rgb_to_grayscale(images)
        labels = tf.ones(shape=[images.shape[0], 1])
        tic = time.time()
    ...
    for test_images, _ in tqdm(val_data, desc="Validation"):
        test_labels = labels = tf.ones(shape=[test_images.shape[0], 1])
