...
q_class = test_df["class"].astype(int)
...
checkpoint_dir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-quaternions-100-classes/checkpoints/"
...
for test_images, test_labels in tqdm(test_data, desc="Testing"):                                  
    ...
    gt.append(tf.squeeze(tf.argmax(test_labels, axis=-1)).numpy())
...
errors = [quaternion_angle(gt_quaternions[i], pred_quaternions[i]) for i in range(len(gt_quaternions))]
...
plt.grid(True)
plt.savefig("accuracy_quaternions_classification2.png")
