...
if __name__ == "__main__":
    ...
    for m in range(M_theta):
        ...
        for n in range(M_phi):
            ...
            for l in range(-20, 21, 10):
                rho = l/180.0 * np.pi
                N_count += 1
                viewpoints.append((float(N_count), float(format(theta, ".10f")), float(format(rho, ".10f")) , float(format(phi, ".10f")))) 
    ...
    for ctID in trainScans:
        ...
        def render_view(viewpoint):
            ...
            rotationMatrix = Ry(theta_y) @ Rx(theta_x) @ Rz(theta_z) 
        ...
        with Pool() as pool:
            pool.map(render_view, viewpoints)
        #     labels_x.append(viewpoint[0])
        #     labels_y.append(viewpoint[1])
