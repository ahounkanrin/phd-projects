...
# Load dataset
data_dir = "/scratch/hnkmah001/Datasets/ctfullbody/roll-pitch-yaw-regular/test/"
test_df = pd.read_csv("test.csv", sep=",")
img_path = test_df["image"].apply(lambda imgID: data_dir+imgID)
label_roll = test_df["azimuth"].astype(float)
label_pitch = test_df["elevation"].astype(float)
label_yaw = test_df["inplane"].astype(float)
...
checkpoint.restore(manager.checkpoints[-6])
model.summary()
...
plt.grid(True)
plt.savefig("accuracy_reg_quaternions3z.png")