...
if __name__ == "__main__":
	...
	img = np.pad(img, N, "constant", constant_values=0)
	print(img.shape)
	...
	x = np.linspace(-3*N//2+0.5, 3*N//2-0.5, 3*N)
	y = np.linspace(-3*N//2+0.5, 3*N//2-0.5, 3*N)
	...
	projectionLine = np.reshape(projectionLine, (3*N, 2, 1))
	...
	plt.figure()
	# plt.title(r"Projection onto the x-axis (no padding)")
	plt.plot(img_x, label="AP")
	plt.plot(projection_x_spline, label=r"FVR")
	plt.legend(loc="upper left")
	plt.grid(True)
	plt.savefig("ap_fvr_x3.png")


	plt.figure()
	# plt.title(r"Projection onto the x-axis (no padding)")
	plt.plot(img_y, label="AP")
	plt.plot(projection_y_spline, label=r"FVR")
	plt.legend(loc="upper left")
	plt.grid(True)
	plt.savefig("ap_fvr_y3.png")
