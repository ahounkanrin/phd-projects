...
def render_train_view(viewpoint):
    ...
    if scale == 80:
        img = cv.resize(img, (410, 410), interpolation=cv.INTER_AREA)
        img = np.pad(img, 51, "constant", constant_values=0)
    if scale == 90:
        img = cv.resize(img, (460, 460), interpolation=cv.INTER_AREA)
        img = np.pad(img, 26, "constant", constant_values=0)
    if scale == 110:
        img = cv.resize(img, (564, 564), interpolation=cv.INTER_AREA)
        img = img[26:538, 26:538]
    if scale == 120:
        img = cv.resize(img, (614, 614), interpolation=cv.INTER_AREA)
        img = img[56:568, 56:568]
...
xtrainval = [(theta, tx, ty) for theta in range(360) for tx in range(-20, 21) for ty in range(-20, 20)]
...
checkpoint_dir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-3d/geom-loss-rotations-aug/checkpoints/"
...
for epoch in range(args.epochs):
    ...
    while len(xtrain_epoch) > 0:
        ...
        if step == 0:
            train_logdir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-3d/geom-loss-rotations-aug/logs/train"
            val_logdir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-3d/geom-loss-rotations-aug/logs/val"
