...
for view_id in range(0, 180, 10):
    ...
    for scale in scales:
        ...
        for ty in tqdm(range(-20, 21, 5), desc="ty"):
            ...
            entropy_arr.append(entropy_list)
        ...
        plt.figure()
        plt.title("Entropy heatmap - scale: {}%".format(scale))
        plt.xlabel("tx (pixles)")
        plt.ylabel("ty (pixels)")
        sns.heatmap(entropy_arr, xticklabels=tx_list, yticklabels=ty_list, vmin=3.5, vmax=5) # 
        plt.xlabel("tx (pixles)")
        plt.ylabel("ty (pixels)")
