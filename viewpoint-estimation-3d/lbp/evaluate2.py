...
scales = ["s50", "s60", "s75", "s80", "s90", "s100", "s110", "s125"]

for view_id in range(0, 180, 5):
    for scale in scales:
        if not os.path.isdir("view{}".format(view_id)):
            os.mkdir("view{}".format(view_id))
        img_test = cv.imread("/scratch/hnkmah001/Datasets/ctfullbody/test/{}/test{}.png".format(scale, view_id), 0)
        #f = open("errors.txt", "+a")
        err_arr = []
