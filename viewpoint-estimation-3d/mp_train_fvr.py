...
def render_train_view(viewpoint):
    ...
    ptsProjectionPlane = np.array([rotate_vector(np.array([xi, 0, zi]), rotationMatrix) for xi in x_axis for zi in z_axis])
    ptsProjectionPlaneInterpolated =  train_interpolating_function(ptsProjectionPlane) 
...
def render_val_view(viewpoint):
    ...
    rotationMatrix = rotation_matrix(theta)
    ptsProjectionPlane = np.array([rotate_vector(np.array([xi, 0, zi]), rotationMatrix) for xi in x_axis for zi in z_axis])
    ptsProjectionPlaneInterpolated =  val_interpolating_function(ptsProjectionPlane) 
    projectionPlane = np.reshape(ptsProjectionPlaneInterpolated, (N,N), order="F")     
    projectionPlaneIFFT = np.abs(np.fft.ifft2(projectionPlane))
    img = np.fft.fftshift(projectionPlaneIFFT)
    img = normalize(img)
...
def rotate_vector(vector, rotationMatrix):
    rotatedVector = np.matmul(rotationMatrix, vector)
