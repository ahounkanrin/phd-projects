...
# Define the model
inputs = tf.keras.Input(shape=(4356,)) # | (20736,)  | (900,)
...
checkpoint_dir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-3d/hog16x16/checkpoints/"
...
for view_id in range(0, 360):
	...
	for scale in scales:
		...
		for ty in tqdm(range(1), desc="ty"):
			for tx in tqdm(range(1), desc="tx"):
				...
				x_test = hog(img, orientations=9, pixels_per_cell=(16, 16), cells_per_block=(2, 2), visualize=False, multichannel=None)
	...
	prediction = np.mean(pred_list)
	gt = view_id
	error = angular_distance(gt, prediction) 
...
for theta in thresholds:
    acc_bool = np.array([err_list[i] <= theta  for i in range(len(err_list))])
