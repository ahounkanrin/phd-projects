...
def render_test_view(viewpoint):
    ...
    hogFeature = hog(img, orientations=9, pixels_per_cell=(64, 64),
                     cells_per_block=(2, 2), visualize=False, multichannel=None)
    label = one_hot_encoding(theta)
    return hogFeature, label

xtest = [(theta, 0, 0) for theta in range(360)]

# Define the model
inputs = tf.keras.Input(shape=(144,)) # | (20736,)  | (900,) | (4356,)
...
checkpoint_dir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-3d/hog64x64/checkpoints/"
if not os.path.isdir(checkpoint_dir):
    os.mkdir(checkpoint_dir)
manager = tf.train.CheckpointManager(checkpoint, directory=checkpoint_dir, max_to_keep=10)

checkpoint.restore(manager.checkpoints[-1])  
#checkpoint.restore("/scratch/hnkmah001/phd-projects/viewpoint-estimation-3d/hog/checkpoints/ckpt-0")    
...
for theta in thresholds:
	...
	acc = np.mean(np.array([errors[i] <= theta  for i in range(len(errors))]))
	#acc_list.append(acc)
	#print("length", len(acc_list))
