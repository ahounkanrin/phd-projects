...
while len(xtest_epoch) > 0:
    ...
    for i in range(len(test_batch)):
        ...
        hog_test.append(test_batch[i, 2])
...
test_data = tf.data.Dataset.from_tensor_slices((x_test, y_test, hog_test)).batch(1)

pred = []
for test_images, test_labels, test_features in tqdm(test_data, desc="Testing"):
    pred_it = test_step([test_images, test_features], test_labels)
