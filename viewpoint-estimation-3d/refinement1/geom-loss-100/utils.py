...
#     angle = int(angle)
#     assert angle<180, "viewpoint angle must be less than 360"
...
def one_hot_encoding(angle):
    angle = int(angle)-80
    assert angle<20, "viewpoint angle must be less than 360"
    labels = np.zeros(20)
    labels[angle] = 1.0
    return labels
...
def get_weights(gt_class, sigma=7.0): 
    k = tf.constant([i for i in range(20)], dtype=tf.float32)
    gt = gt_class * tf.ones(shape=(20))
