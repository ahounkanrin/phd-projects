...
def render_test_view(viewpoint):
    ...
    return img, label
#for ty in tqdm(range(-20, 21), desc="ty"):
#    for tx in tqdm(range(-20, 21), desc="tx"):
...
for ty in tqdm(range(-20, 21), desc="ty"):
    ...
    for tx in tqdm(range(-20, 21), desc="tx"):
        ...
        f = open("acc.txt", "a+")
        xtest = [(theta, tx, ty) for theta in range(360)]
        ...
        theta = 30
        mederr = np.median(np.array(errors))
        mederr_list.append(mederr)
        print("tx = {} and ty = {}".format(tx, ty))
        print("Median Error = {:.4f}".format(mederr))

        print("\n\ntx = {} and ty = {}".format(tx, ty), file=f)
        print("Median Error = {:.4f}".format(mederr), file=f)
    ...
    mederr_arr.append(mederr_list)
acc_arr = np.array(acc_arr)
mederr_arr = np.array(mederr_arr)
tx_list = [i for i in range(-20, 21)]
ty_list = [i for i in range(-20, 21)]

sns.set(font_scale=3)
plt.figure(figsize=(30, 20))
sns.heatmap(acc_arr, annot=True, annot_kws={"size": 40}, xticklabels=tx_list, yticklabels=ty_list)
plt.savefig("accuracy_heatmap.png")

plt.figure(figsize=(30, 20))
sns.heatmap(mederr_arr, annot=True, annot_kws={"size": 40}, xticklabels=tx_list, yticklabels=ty_list)
plt.savefig("mederr_heatmap.png")