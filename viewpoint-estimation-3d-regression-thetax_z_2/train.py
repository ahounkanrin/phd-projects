...
import random
from utils import trigMSE, euler_to_quaternion, quaternionDistance
...
def generate_train_data(viewpoint):
    ...
    roll, pitch, yaw = theta_z, theta_x, theta_y
    #print(euler_to_quaternion([roll, pitch, yaw]))
    return img, np.array(euler_to_quaternion([roll, pitch, yaw]))
...
def select_viewpoints(batch_size):
    ...
    for i in range(batch_size):
        theta_x = np.random.uniform(low=-np.pi/2, high=np.pi/2, size=None)
        theta_y = np.random.uniform(low=-np.pi, high=np.pi, size=None)
        theta_z = np.random.uniform(low=-np.pi, high=np.pi, size=None)
        batch.append([theta_x, theta_y, theta_z])
    return batch

def normalizeVector(q, normq):
    if normq == 0.0:
        normq = 1e-30
    return q/normq
...
def train_step(images, labels):
    ...
    with tf.GradientTape() as tape:
        ...
        loss = quaternionDistance(rawpreds, labels)
...
while True:
    ...
    if step == 0:
        train_logdir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-3d-regression-quaternions/logs/train"
        val_logdir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-3d-regression-quaternions/logs/val"
