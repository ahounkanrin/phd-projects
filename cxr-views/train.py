...
voi = normalize(voi)    # Rescale CT numbers into 0-255 range
print("Done.") 
...
def render_train_view(viewpoint):
    ...
    if (0 <= theta <= 20) or (160 <= theta <= 200) or (340 <= theta <= 359):  # Frontal views
        label = one_hot_encoding(0)
    elif (80 <= theta <= 105) or (255 <= theta <= 280): # Lateral views
...
x11 = [(theta, tx, 0) for theta in range(21) for tx in range(-20, 21)]
x12 = [(theta, 0, ty) for theta in range(21) for ty in range(-20, 21) if ty!=0]
x21 = [(theta, tx, 0) for theta in range(160, 201) for tx in range(-20, 21)]
x22 = [(theta, 0, ty) for theta in range(160, 201) for ty in range(-20, 21) if ty!=0]
x31 = [(theta, tx, 0) for theta in range(340, 360) for tx in range(-20, 21)]
x32 = [(theta, 0, ty) for theta in range(340, 360) for ty in range(-20, 21) if ty!=0]
x41 = [(theta, tx, 0) for theta in range(80, 106) for tx in range(-20, 21)]
x42 = [(theta, 0, ty) for theta in range(80, 106) for ty in range(-20, 21) if ty!=0]
x51 = [(theta, tx, 0) for theta in range(255, 281) for tx in range(-20, 21)]
x52 = [(theta, 0, ty) for theta in range(255, 281) for ty in range(-20, 21) if ty!=0]
xtrainval = x11 + x12 + x21 + x22 + x31 + x32 + x41 + x42 + x51 + x52
...
for epoch in range(args.epochs):
    ...
    while len(xtrain_epoch) > 0:
        ...
        if step == 0:
            train_logdir = "/scratch/hnkmah001/phd-projects/cxr-views/logs/train"
            val_logdir = "/scratch/hnkmah001/phd-projects/cxr-views/logs/val"
    ...
    print(template.format(epoch, test_loss.result(), test_accuracy.result(), ckpt_path))
