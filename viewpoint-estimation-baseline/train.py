...
def select_viewpoints(batch_size):
    ...
    for i in range(batch_size):
        theta_x = np.random.uniform(low=-30, high=30) 
        theta_y = np.random.uniform(low=-30, high=30)
...
outputx = tf.multiply(outputx, tf.constant(30.0))
...
model = tf.keras.Model(inputs=inputs, outputs=[outputx, outputy, outputz])
model.summary()
...
def train_step(images, labels):
    ...
    with tf.GradientTape() as tape:
        predx, predy, predz = model(images, training=True)
        lossx = loss_objectx(labels, predx)
        lossy = loss_objecty(labels, predy)
        lossz = loss_objectz(labels, predz)
        loss = tf.divide(lossx, tf.constant(60.0**2)) + tf.divide(lossy, tf.constant(60.0**2)) + tf.divide(lossz, tf.constant(360.0**2))
...
checkpoint_dir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-baseline/checkpoints/"
...
while True:
    ...
    if step == 0:
        train_logdir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-baseline/logs/train"
        val_logdir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-baseline/logs/val"
    ...
    if step % 10000 == 0:
        ...
        x_val = []
        y_val = []
        val_viewpoints_batch = select_viewpoints(1000*args.batch_size)
        with Pool() as pool:
        ...
        test_loss = test_lossx.result()/(60.0**2) + test_lossy.result()/(60.0**2) + test_lossz.result()/(360.0**2)
