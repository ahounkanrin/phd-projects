...
for scan in testScans:
    ...
    for view_id in tqdm(range(0, nclasses), desc="\n{}".format(scan)):
        err_list = []
        #for scale in scales:
        img_test = cv.imread("/scratch/hnkmah001/Datasets/ctfullbody/ctfullbody2d/data-rz/normals/test/{}/s{}/{}.png".format(scan, scale, view_id), 0)
        for ty in range(1):
            for tx in range(1):
...
test_acc_mean = np.mean(test_acc_list, axis=0)
print("Test accuracy = ", test_acc_mean)
f = open("acc.txt", "w+")
print("acc = ",  test_acc_mean, file=f)
f.close()