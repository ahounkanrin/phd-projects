...
import os
import time
from utils import euler_to_quaternion, quaternionAngle, quaternionLoss
num_threads = 8
...
y_train_el= y1_el
y_train_az= y1_az
y_train_ip = np.zeros_like(y_train_az)
y_test_el = y2_el
y_test_az = y2_az
y_test_ip = np.zeros_like(y_test_az)

assert max(y_train_az) == 350 and max(y_train_el) == 170
...
y_train = np.stack([y_train_az, y_train_el, y_train_ip], axis=-1)
y_test =  np.stack([y_test_az, y_test_el, y_test_ip], axis=-1)
y_train = (np.pi/180.0) * y_train # convert to randians
y_test = (np.pi/180.0) * y_test
y_train = np.array([euler_to_quaternion(i) for i in y_train])
...
train_logdir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-roll-pitch/logs/train"
val_logdir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-roll-pitch/logs/val"
...
for epoch in range(args.epochs):
    for images, labels in train_data:
        ...
        toc = time.time()
        print("Step {}: \t loss = {:.4f}  \t({:.2f} seconds/step)".format(step, train_loss, toc-tic))
