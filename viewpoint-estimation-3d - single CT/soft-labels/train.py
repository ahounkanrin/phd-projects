...
for epoch in range(args.epochs):
    ...
    while len(xtrain_epoch) > 0:
        ...
        for images, labels in train_data.map(preprocess, num_parallel_calls=tf.data.experimental.AUTOTUNE):
            ...
            with train_summary_writer.as_default():
                ...
                tf.summary.image("image", images, step=step, max_outputs=4)
            toc = time.time()
    ...
    x_val = []
    y_val = []
    print("\n\n Epoch {}: Generating validation data...".format(epoch))
    while len(xval_epoch) > 0:
        if len(xval_epoch) >= cpu_count():
            val_viewpoints_batch = random.sample(xval_epoch, cpu_count())
        else:
    ...
    with val_summary_writer.as_default():
        ...
        tf.summary.scalar("val loss", test_loss.result(), step=epoch)
        tf.summary.image("val_images", val_images, step=epoch, max_outputs=4)

    ckpt_path = manager.save()
    template = "Epoch {}: \t Validation Accuracy ={:.4f}, \t ckpt {}"
    print(template.format(epoch, test_accuracy.result(), ckpt_path))
