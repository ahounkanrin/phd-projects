...
import time
import pandas as pd
from utils import euler_to_quaternion, quaternionLoss, quaternionAngle
...
# Load dataset
data_dir = "/scratch/hnkmah001/Datasets/ctfullbody/quaternions_5000_classes2/test/"
test_df = pd.read_csv("test_2000.csv", sep=",")
...
labels_list = tf.constant(np.array([[qw[i], qx[i], qy[i], qz[i]] for i in range(len(qw))]))
test_data = tf.data.Dataset.from_tensor_slices((img_path_list, labels_list)).batch(1)
...
checkpoint_dir = "/scratch/hnkmah001/phd-projects/viewpoint-estimation-roll-pitch-yaw-quaternions2/checkpoints/"
...
for test_images, test_labels in tqdm(test_data.map(preprocess, 
    ...
    # print("prediction", tf.squeeze(preds).numpy())
    # print("gt", tf.squeeze(test_labels).numpy())
    # print("error", quaternionAngle(test_labels.numpy(), tf.squeeze(preds).numpy()))
    # print("loss", loss)

errors = [quaternionAngle(gt[i], preds_list[i]) for i in range(len(gt))]
...
plt.grid(True)
plt.savefig("accuracy_reg_quaternions3.png")
