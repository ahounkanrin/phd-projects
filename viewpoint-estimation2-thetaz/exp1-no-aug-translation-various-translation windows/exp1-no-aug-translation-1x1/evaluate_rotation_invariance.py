...
for view_id in tqdm(range(0, 360), desc="\n{}".format(scan)):
    ...
    for angle in rotation_angles:
        errors_txty = []
        img_rotated = imutils.rotate(img_test, angle)
        for ty in range(-20, 21, 5): # Try -20, 21, 2
            for tx in range(-20, 21, 5):
                img = img_rotated[56+ty:456+ty, 56+tx:456+tx]
                ...
                errors_txty.append(error)
        errors_theta.append(np.min(errors_txty))
    optimal_rot_angles.append(rotation_angles[np.argmin(errors_theta)])        
                
    min_errors.append(np.min(errors_theta))

print("Optimal rotation angles", optimal_rot_angles)            
