...
for i in tqdm(range(len(train_img_path))): # len(train_img_path))
    ...
    x_train[i] = img
    y_train.append(int(train_img_label[i]))
...
for i in tqdm(range(len(val_img_path))):  # len(val_img_path)
    ...
    x_val[i] = img
    y_val.append(int(val_img_label[i]))
...
for i in tqdm(range(len(test_img_path))):
    ...
    x_test[i] = img
    y_test.append(int(test_img_label[i]))
y_test = np.array(y_test)

save_dataset(x_train, y_train,x_val, y_val, x_test, y_test, DIR + 'chest_fov_400x400_sparse_labels.h5')
